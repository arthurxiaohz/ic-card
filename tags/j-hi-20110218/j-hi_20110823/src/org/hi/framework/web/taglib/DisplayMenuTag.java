    ...
    if (repository == null) {
		...
		if(menu == null && menuName != null){
			...
			try {
				...
				MenuComponent _menu = null;
				if(type.equals("lazy") || type.equals("subMenu"))
					_menu = ((WebDynamicTreeManager)treemgr).getLoadMenu(menuName, parMap,type == null ? "commonloadxtree":type,(HttpServletRequest)pageContext.getRequest());
				else
					_menu = treemgr.getMenu(menuName,parMap,(HttpServletRequest)pageContext.getRequest());
				pageContext.setAttribute(this.name,_menu);
			} catch (Exception e) {
				e.printStackTrace();
			}
