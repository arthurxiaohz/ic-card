...
public abstract class BaseObject implements Serializable {  
	...
	/**
	 * 如果在POJO中某个属性的值被改变，那么可以通过该方法获取变更前的原值
	 * @param propertyName 属性名
	 * @return 返回改变之前给定属性名的属性值
	 */
	public final Object getOldValue(String propertyName){
	 ...
	 * 通过给定属性名判断该属性的值是否被改变
	 * @param propertyName 属性名
	 * @return 如果已变更返回true,否则为false
	 */
	public final boolean hasOldValue(String propertyName){
		return oldValues.keySet().contains(propertyName);
	}
	
	/**
	 * 遍历所有从实体包括自已,如果有任何一个为dirty则返回true
	 * @return
	 */
	public final boolean isCascadeDirty(){
		if(this.dirty)
			return true;
