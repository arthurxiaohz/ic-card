...
public class PayResponseService implements IPayResponseService {
	...
	public String savePayResponse(int id) throws Exception {
		...
		if(tblMchtFee.getMchtFeeType() == MchtFeeType.MCHTFEETYPE_BYRATE){
			//按比例
			mchtFee = ((new BigDecimal(tblTxPayMentOrder.getOrderAmount()).multiply(new BigDecimal(tblMchtFee.getRuleValue()))).setScale(0, BigDecimal.ROUND_HALF_UP)).intValue();   //舍掉小数
		...
		if(pointConfig.getPointRuleType()== PointRuleType.POINTRULETYPE_BYRATE){
			//按比例
			point = ((new BigDecimal(tblTxPayMentOrder.getPayAmount()).multiply(new BigDecimal(pointConfig.getPointRuleType()))).setScale(0, BigDecimal.ROUND_HALF_UP)).intValue();   //舍掉小数
