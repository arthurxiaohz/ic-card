...
<div class="pageHeader">
	...
	<div class="searchBar">
		<ul class="searchContent">	
			...
			<li>
				<label><hi:text key="会员账号" entity="TblTxTransfer"/>:</label>
	...
	<div class="panelBar">
		...
		<c:choose>
			<c:when test="${empty lookup}">
				<authz:authorize ifAnyGranted="TBLTXTRANSFER_SAVE"><li><a class="add" href="<hi:url>tblTxTransferEdit.action?tblTxTransfer.id=-1</hi:url>" target="navTab" rel="tblTxTransfer"><span><hi:text key="新建" parameterLanguageKeys="转账交易"/></span></a></li></authz:authorize>
			...
			<c:otherwise>
				<li><a class="icon" href="javascript:$.bringBack({id:'-1', plTxTraceNo:'',userName:'',txTypeId:'',mchtNo:'',mchtTxTime:'',mchtTxTraceNo:'',txAmount:'',txStatus:'',errorCode:'',createdDatetime:''})"><span><hi:text key="重置"/></span></a></li>
			</c:otherwise>
		</c:choose>			
		</ul>
	</div>
	<table class="table" width="100%" layoutH="138" targetType="${targetType}">
		<thead>
			<tr>
				...
				<th orderField="userName" class="${pageInfo.sorterName eq 'userName' ? pageInfo.sorterDirection : ''}"><hi:text key="会员账号" entity="TblTxTransfer"/></th>
			...
			<tr>
				...
				<c:choose>
				    ...
				    <authz:authorize ifAnyGranted="TBLTXTRANSFER_DEL">
				      <a class="btnDel" href="<hi:url>tblTxTransferRemove.action?ajax=1&tblTxTransfer.id=${item.id}</hi:url>" target="navTabTodo" title="<hi:text key="删除" parameterLanguageKeys="转账交易"/>"><hi:text key="删除"/></a>
				    </authz:authorize>
				    <authz:authorize ifAnyGranted="TBLTXTRANSFER_VIEW">
				      <a class="btnView" href="<hi:url>tblTxTransferView.action?tblTxTransfer.id=${item.id}</hi:url>" target="navTab" rel="tblTxTransfer" title="<hi:text key="查看" parameterLanguageKeys="转账交易"/>"><hi:text key="查看"/></a>
				    </authz:authorize>
				    <authz:authorize ifAnyGranted="TBLTXTRANSFER_SAVE">
				      <a class="btnEdit" href="<hi:url>tblTxTransferEdit.action?tblTxTransfer.id=${item.id}</hi:url>" target="navTab" rel="tblTxTransfer" title="<hi:text key="编辑" parameterLanguageKeys="转账交易"/>"><hi:text key="编辑"/></a>
				    </authz:authorize>
					</c:when>
					<c:otherwise>
						<a class="btnSelect" href="javascript:$.bringBack({id:'${item.id}', plTxTraceNo:'${item.plTxTraceNo}',userName:'${item.userName}',txTypeId:'${item.txTypeId}',mchtNo:'${item.mchtNo}',mchtTxTime:'${item.mchtTxTime}',mchtTxTraceNo:'${item.mchtTxTraceNo}',txAmount:'${item.txAmount}',txStatus:'<hi:select emu="transTxStatus" name="tblTxTransfers[${s.index}].txStatus" isLabel="true"/>',errorCode:'${item.errorCode}',createdDatetime:'${item.createdDatetime}'})" title="<hi:text key="查找带回"/>"><hi:text key="选择"/></a>
					</c:otherwise>
				</c:choose>
				</td>
			</tr>
