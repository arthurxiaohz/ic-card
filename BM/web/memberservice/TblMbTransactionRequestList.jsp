...
<div class="pageContent">
	<div class="panelBar">
		...
		<c:choose>
			...
			<c:otherwise>
				<li><a class="icon" href="javascript:$.bringBack({id:'-1', trancode:'',mchtNo:'',amount:'',trxTime:'',txStatus:'',createdDatetime:'',pan:'',plTxTraceNo:'',plTxTime:'',orderId:''})"><span><hi:text key="重置"/></span></a></li>
			</c:otherwise>
		</c:choose>			
		</ul>
	</div>
		...
		<tbody>
			...
			<tr>
				...
				<td><input name="orderIndexs" value="${item.id}" type="checkbox"></td>
				    ...
				    <td>${item.amount}</td>
				    <td>${item.trxTime}</td>
				    <td><hi:select emu="rechargeTxStatus" name="tblMbTransactionRequests[${s.index}].txStatus" isLabel="true"/></td>
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="btnSelect" href="javascript:$.bringBack({id:'${item.id}', trancode:'${item.trancode}',mchtNo:'${item.mchtNo}',amount:'${item.amount}',trxTime:'${item.trxTime}',txStatus:'<hi:select emu="rechargeTxStatus" name="tblMbTransactionRequests[${s.index}].txStatus" isLabel="true"/>',createdDatetime:'${item.createdDatetime}',pan:'${item.pan}',plTxTraceNo:'${item.plTxTraceNo}',plTxTime:'${item.plTxTime}',orderId:'${item.orderId}'})" title="<hi:text key="查找带回"/>"><hi:text key="选择"/></a>
					</c:otherwise>
				</c:choose>
				</td>
			</tr>
