...
<form id="pagerForm" action="tblActAccountBalanceList.action">
...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="tblActAccountBalanceList.action?lookup=${lookup}" onsubmit="return dwzSearch(this, '${targetType}');">
	<input type="hidden" name="pageInfo.pageSize" value="${pageInfo.pageSize}" />
	<div class="searchBar">
		<ul class="searchContent">	
			<li>
				<label><hi:text key="可用余额" entity="TblActAccountBalance"/>:</label>
				<input type="text" name="pageInfo.f_availableBalance" value="${pageInfo.f_availableBalance}"/>
			</li>	  
			<li>
				<label><hi:text key="账号" entity="TblActAccountBalance"/>:</label>
				<input type="text" name="pageInfo.f_accountNo" value="${pageInfo.f_accountNo}"/>
			</li>	  
			<li>
				<label><hi:text key="账户分类" entity="TblActAccountBalance"/>:</label>
				<hi:search name="pageInfo.f_accountCatalog" emu="accountCatalog"/>
			</li>	  
			<li>
				<label><hi:text key="开户方类型" entity="TblActAccountBalance"/>:</label>
				<hi:search name="pageInfo.f_accountPartyType" emu="accountPartyType"/>
			</li>	  
			<li>
				<label><hi:text key="开户方" entity="TblActAccountBalance"/>:</label>
				<input type="text" name="pageInfo.f_accountParty" value="${pageInfo.f_accountParty}"/>
			</li>	  
			<li>
				<label><hi:text key="开户名称" entity="TblActAccountBalance"/>:</label>
				<input type="text" name="pageInfo.f_accountName" value="${pageInfo.f_accountName}"/>
			</li>	  
			<li>
				<label><hi:text key="备注" entity="TblActAccountBalance"/>:</label>
				<input type="text" name="pageInfo.f_remark" value="${pageInfo.f_remark}"/>
			</li>	  
			<li class="dateRange">
				<label><hi:text key="创建时间" entity="TblActAccountBalance"/>:</label>
			...
			<li class="dateRange">
				<label><hi:text key="最后修改时间" entity="TblActAccountBalance"/>:</label>
	...
	<table class="table" width="100%" layoutH="138" targetType="${targetType}">
		<thead>
			<tr>
				...
				<th orderField="availableBalance" class="${pageInfo.sorterName eq 'availableBalance' ? pageInfo.sorterDirection : ''}"><hi:text key="可用余额" entity="TblActAccountBalance"/></th>
				<th orderField="accountNo" class="${pageInfo.sorterName eq 'accountNo' ? pageInfo.sorterDirection : ''}"><hi:text key="账号" entity="TblActAccountBalance"/></th>
				<th orderField="accountCatalog" class="${pageInfo.sorterName eq 'accountCatalog' ? pageInfo.sorterDirection : ''}"><hi:text key="账户分类" entity="TblActAccountBalance"/></th>
				<th orderField="accountPartyType" class="${pageInfo.sorterName eq 'accountPartyType' ? pageInfo.sorterDirection : ''}"><hi:text key="开户方类型" entity="TblActAccountBalance"/></th>
				<th orderField="accountParty" class="${pageInfo.sorterName eq 'accountParty' ? pageInfo.sorterDirection : ''}"><hi:text key="开户方" entity="TblActAccountBalance"/></th>
				<th orderField="accountName" class="${pageInfo.sorterName eq 'accountName' ? pageInfo.sorterDirection : ''}"><hi:text key="开户名称" entity="TblActAccountBalance"/></th>
				<th orderField="status" class="${pageInfo.sorterName eq 'status' ? pageInfo.sorterDirection : ''}"><hi:text key="状态" entity="TblActAccountBalance"/></th>
				<th orderField="remark" class="${pageInfo.sorterName eq 'remark' ? pageInfo.sorterDirection : ''}"><hi:text key="备注" entity="TblActAccountBalance"/></th>
				<th orderField="createdDateTime" class="${pageInfo.sorterName eq 'createdDateTime' ? pageInfo.sorterDirection : ''}"><hi:text key="创建时间" entity="TblActAccountBalance"/></th>
				<th orderField="lastUpdatedDatetime" class="${pageInfo.sorterName eq 'lastUpdatedDatetime' ? pageInfo.sorterDirection : ''}"><hi:text key="最后修改时间" entity="TblActAccountBalance"/></th>
				<th width="90">
					<c:choose>
		...
		<tbody>
			<c:forEach var="item" items="${tblActAccountBalances}" varStatus="s">
			<tr>
				<c:if test="${empty lookup}">
				<td><input name="orderIndexs" value="${item.id}" type="checkbox"></td>
				    ...
				    <td><hi:select emu="accountStatus" name="tblActAccountBalances[${s.index}].status" isLabel="true"/></td>
				    <td>${item.remark}</td>
