...
public class BeanUtil {
	...
	public static String getCollection2JSON(Collection col, String collName , String properies, boolean isObject){
		...
		sb.append(getCollection2JSONValue(col, collName, properies, 0));
	...
	protected static String getCollection2JSONValue(Collection col, String collName , String properies, int level){
		...
		for (Object object : col) {
			if(i >0)
				sb.append(",");
			i++;
			sb.append(getBean2JSON(object, properies, level));
		}
		sb.append("]");
	...
	private static String getBean2JSON(Object bean, String properies, int level){
		...
		for(Map.Entry<String, List<String>> entry: map.entrySet()) {
			...
			if(Collection.class.isAssignableFrom(propertyClazz)){
				...
				sb.append(getCollection2JSONValue((Collection)val, propertyName, collProperties, ++level));
				level--;
			}
				...
				if(val == null)
					...
					sb.append(StringUtils.escapeHtml(getPropertyValueToStr(bean, propertyName)));	//先做html转义再作为JSON的值
				}
