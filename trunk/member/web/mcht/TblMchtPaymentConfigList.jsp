...
<div class="pageContent">
	<div class="panelBar">
		...
		<c:choose>
			...
			<c:otherwise>
				<li><a class="icon" href="javascript:$.bringBack({id:'-1', authorized:'',signType:'',md5:''})"><span><hi:text key="重置"/></span></a></li>
			</c:otherwise>
		</c:choose>			
		</ul>
	</div>
		...
		<tbody>
			...
			<tr>
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="btnSelect" href="javascript:$.bringBack({id:'${item.id}', authorized:'<hi:select emu="yesNo" name="tblMchtPaymentConfigs[${s.index}].authorized" isLabel="true"/>',signType:'<hi:select emu="signType" name="tblMchtPaymentConfigs[${s.index}].signType" isLabel="true"/>',md5:'${item.md5}'})" title="<hi:text key="查找带回"/>"><hi:text key="选择"/></a>
					</c:otherwise>
				</c:choose>
				</td>
			</tr>
